...
namespace ticl {
  ...
  public:
    SuperTrackster() : rawEnergy_(0.f), hasTrack(false) {}

    SuperTrackster(const std::vector<uint32_t>& tracksters) : tracksterIdxs_({tracksters}) {}

    SuperTrackster(const std::vector<uint32_t>& tracksters, const uint32_t& track)
        : tracksterIdxs_({tracksters}), trackIdx_(track), hasTrack(true) {}
    ...
    void addTrackster(const uint32_t tracksterIdx) { tracksterIdxs_.push_back(tracksterIdx); }
