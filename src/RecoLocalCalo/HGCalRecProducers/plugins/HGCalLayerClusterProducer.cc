...
void HGCalLayerClusterProducer::produce(edm::Event& evt,
  ...
  //std::vector<edm::ESHandle<HGCalTopology>&> topo_array;
  //const HGCalTopology& topo_array[3];
  
  es.get<IdealGeometryRecord>().get("HGCalEESensitive",topo);
  const auto& topologyEE = *topo;
  if (topo.isValid()) std::cout <<"EE topo is valid \n";

  es.get<IdealGeometryRecord>().get("HGCalHESiliconSensitive",topo);
  const auto& topologySi = *topo;
  if (topo.isValid()) std::cout <<"HE Si topo is valid \n";

  es.get<IdealGeometryRecord>().get("HGCalHEScintillatorSensitive",topo);
  const auto& topologyScin = *topo;
  if (topo.isValid()) std::cout <<"HE Scin topo is valid \n";
  ...
  /*for (auto& x : u){
    std::cout << (uint32_t)x.first <<" : "<< x.second << std::endl;
    }*/

  std::cout << "# in unordered_map = " << u.size() << std::endl;
  auto start2 = std::chrono::high_resolution_clock::now();
  algo->makeClusters(topologyEE, topologySi, topologyScin);
  auto finish2 = std::chrono::high_resolution_clock::now();

  std::cout  <<"Duration from LayerClusterProducer = "<< (std::chrono::duration<double>(finish2-start2)).count() << std::endl;
